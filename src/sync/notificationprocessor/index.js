...
export default function NotificationProcessorFactory(callbacks, userKeyHashes) {
  ...
  function handleEvent(eventData, channel) {
    switch (eventData.type) {
      ...
      // @TODO check if we should call pushmanager.scheduleReAuth or something like that. As we do when Auth fails due to HTTP or Network errors.
      case EventTypes.SSE_ERROR:
        // HTTP or Network error associated to streaming endpoint. We must close conexion to avoid error loop in SSE connection
  ...
  return {
    handleOpen() {
      ...
      callbacks.stopPolling(); // needed on success reauth after a fail auth. In other scenarios, stopPolling will do nothing (i.e., polling mode is already disabled)
      callbacks.syncAll();
    },
